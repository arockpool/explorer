<template>
  <div  v-loading="loadingFlag" class="flex flex-col bg-white px-3 lg:px-24 pt-3 lg:pt-8 pb-3 lg:pb-8 mb-3">
      ...
      <div class="w-full lg:w-108 lg:h-148 border-2 lg:border-4 rounded-xl border-main border-solid lg:flex">
        <div class="flex-1 mx-3 lg:ml-8 lg:mr-6">
          ...
          <div class="mt-5 mb-2 text-040A25">
            <span class="text-040A25 text-xl">满存算力</span>
            <el-popover placement="top-start" trigger="hover" content="存储好的有效算力">
              <i class="el-icon-info text-gray-500" slot="reference"></i>
            </el-popover>
          </div>
          <div>
            <el-input v-model="form.power" placeholder="满存算力">
          ...
          <div class="mt-5 mb-2">
            ...
            <el-popover placement="top-start" trigger="hover" content="获得满存算力所投入的成本">
              <i class="el-icon-info text-gray-500" slot="reference"></i>
            </el-popover>
          </div>
          <div class="mb-5 lg:mb-59">
          ...
          <template v-if="calculatorType == 2">
            ...
            <div class="mt-3 flex-1 flex items-center">
              <div class="h-full flex flex-col justify-center flex-1 bg-white rounded-md relative" :class="{'ml-3' : index > 0}" v-for="(out, index) in bigOut" :key="index + 100">
                ...
                <div class="text-center text-E9AB1C">
                  <span class="text-5xl font-medium">{{ out.reward }}</span>
                  <span>{{ out.desc }}</span>
                </div>
            ...
            <!--             <div class="mt-3 flex-1 flex items-center">
              <div class="h-full flex-1 bg-white flex flex-col items-center justify-center p-3 rounded-md" :class="{'ml-3' : index > 0}" v-for="(_in, index) in bigIn" :key="index + 1000">
                <div class="text-dark text-lg">{{ _in.title }}</div>
                <div class="text-main">
    ...
    <div class="text-base mt-3 lg:mt-8 leading-7">
      ...
      <div class="text-8F94A8">2.无存储过程，直接以满存算力计算产出。</div>
      <div class="text-8F94A8">3.结果为理论产出，可能与实际产出存在偏差，仅供参考。</div>
      <div class="text-8F94A8">4.计算结果未考虑平台分成。</div>
      <div class="text-8F94A8 block xl:hidden">5.每30min更新数据。</div>
    </div>
    <!-- 分享 -->
    ...
    calculatorHandler: async function() {
      ...
      this.$axios.post('/api/calculator/get_calculate_sum_v2', {
        ...
        .then(res => {
          ...
          let tips_str = {
            'full_day': '按设定的算力机存储速度，存储满存储容量所需的天数和对应的存储区块奖励总产出',
            'win_day': '存储至产出平衡当天的存储区块奖励总产出',
            '540': '存储540天的存储区块奖励总产出'
          }
          let out = []
          let _in = []
          let cost = []
          ...
          _keys.map(item => {
            ...
            switch (item.key) {
              ...
              case '540':
                {
                  _in.push({ title: '维护Gas消耗', value: `${utils.formatterAmount(this.config.keep_cost_gas_per_t, 4)}`, desc: ' F/TiB', tips: '维护算力所消耗的Gas费（根据全网平均值计算得出）' })
                  let _cost = []
                  value.reward.map(a => {
                  ...
                  // })
                  cost.push({ title: '单F成本估算', value: `${_cost[0]}`, desc: ' CNY', tips: '投入成本/存储540天的存储区块奖励总产出，不包含gas相关成本' })
                }
                break;
            }
          })
