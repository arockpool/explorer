<template>
  <div class="lg:relative">
      ...
      <table class="table-fixed w-full">
        <thead>
          <tr class="">
            ...
            <th class="py-4 font-medium text-6C757E">{{$t('Lang.Number of blocks')}}
              <el-popover trigger="hover" :content="$t('Lang.In Filecoin mining model')">
                <i class="el-icon-info cursor-pointer text-gray-500 text-base" slot="reference"></i>
              </el-popover>
            </th>
            <th class="py-4 font-medium text-6C757E">{{$t('Lang.Lucky value')}}</th>
            <th class="py-4 font-medium text-6C757E">{{$t('Lang.Block reward/Proportion')}}</th>
            <th class="py-4 font-medium text-6C757E">{{$t('Lang.Adj Power')}}</th>
        ...
        <tbody>
          <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + '_index'">
            ...
            <!-- 出块数 -->
            <td class="py-3 text-center">{{item.win_count}}</td>
            <!-- 幸运值 -->
            <td class="py-3 text-center">{{item.lucky | formatPercentage}}</td>
            <!-- 奖励/占比 -->
            <td class="py-3 text-center">{{item.block_reward | getBlock_reward(total_block_reward)}}</td>
            <!-- 有效算力 -->
            <td class="py-3 text-center">{{item.power | formatPower()[0]}} {{item.power | formatPower()[1]}}</td>
            <!-- 扇区大小 -->
            <td class="py-3 text-center">
    ...
    <div class="lg:hidden block" v-loading='loading'>
      <div v-for="(item, index) in objs" class="p-3 mb-3  rounded-lg bg-white border border-solid border-E8E7EB" :key="index + 'data_index'">
        ...
        <div class="flex items-center justify-between pb-1">
          <span>{{$t('Lang.Adj Power')}}</span>
          <div>
            <span>{{item.power | formatPower()[0]}} {{item.power | formatPower()[1]}}</span>
          </div>
        </div>
        <div class="flex items-center justify-between pb-1">
          <span>{{$t('Lang.Lucky value')}}</span>
          <div>
            <span>{{item.lucky | formatPercentage}}</span>
          </div>
        </div>
        <div class="flex items-center justify-between pb-1">
          <span>{{$t('Lang.Number of blocks')}}
            <el-popover trigger="hover" :content="$t('Lang.In Filecoin mining model')" width="300">
              <i class="el-icon-info cursor-pointer text-gray-500 text-base" slot="reference"></i>
            </el-popover>
          </span>
          <div>
  ...
  methods: {
    searchRecordHandler(pageIndex) {
      ...
      .then(res => {
        ...
        this.total_block_reward = res.data.total_block_reward
      })
    },
