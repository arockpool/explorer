        ...
        <table class="table-fixed w-full">
          <thead>
            <tr class="">
              ...
              <th class="w-1/12 py-4 font-medium">{{$t('Lang.Power growth rate')}}
        ...
        <table class="table-fixed w-full">
          <thead>
            <tr class="">
              ...
              <th class="w-2/12 py-4 font-medium">{{$t('Lang.Power growth rate')}}
                <el-popover trigger="hover" :content="$t('Lang.The sum of the effective computing power of the packaging sector completed in the last 24 hours')">
                  <i class="el-icon-info cursor-pointer text-gray-500 text-base" slot="reference"></i>
                </el-popover>
              </th>
          ...
          <tbody>
            <tr v-for="(item, index) in objs" class="border-t border-solid border-E8E7EB" :key="index + '_index'">
              ...
              <td class="w-2/12 py-3 text-center">
                <span>{{item.increase_power | formatPower()[0]}} {{item.increase_power | formatPower()[1]}}</span>
              </td>
              <td class='mr-4 text-center'>
...
export default {
  ...
  asyncData({ app }) {
    ...
    //   user_id:data.user_info.data.user_id
    //   })
    // }).then(res=>{
    //   console.log('获取节点收藏',res);
    // })
    return app.$axios
      .post("/api/explorer/get_collectible_miner_list", {
        filter_type: "power_inc",
        page_index: 1,
        page_size: 20,
        stats_type:'24h'
      })
    ...
    getexcel(){
      this.$axios.post('/api/explorer/get_collectible_miner_list', {
        ...
        this.json_data.map(i=>{
          i.power = utils.formatPower(i.power)[0] + utils.formatPower(i.power)[1]
          i.increase_power = utils.formatPower(i.increase_power)[0] + utils.formatPower(i.increase_power)[1] + '/天'
          i.increase_power_offset = utils.formatPower(i.increase_power_offset)[0] + utils.formatPower(i.increase_power_offset)[1]
          i.sector_size = utils.formatPower(i.sector_size)[0] + utils.formatPower(i.sector_size)[1]
        })
      })
    }
  },
