<template>
  <div>
      ...
      <div class='xl:block lg:hidden md:hidden sm:hidden hidden'>
        ...
        <div class='banner  relative' style='height:300px'>
          <div class='introduce absolute text-white' style='height:170px;width:450px;left:8%;bottom:20%'>
            ...
            <div>
              <p class='tracking-wider opacity-75'>{{$t('Lang.From now to x month x day registered members can get the right to use the Pro version of the Arockpool browser worth 666 yuan for one month')}},{{$t('Lang.Invite friends to register to enjoy the right to use the Pro version for another month (superimposable)')}}</p>
            </div>
      ...
      <div class='xl:hidden lg:hidden md:hidden sm:block hidden'>
        <div class='relative banner' style='height:170px'>
          <div class='introduce absolute text-white' style='height:120px;width:370px;left:4%;bottom:22%'>
            ...
            <div>
              <p class='tracking-wider opacity-75 leading-5'>{{$t('Lang.From now to x month x day registered members can get the right to use the Pro version of the Arockpool browser worth 666 yuan for one month')}},{{$t('Lang.Invite friends to register to enjoy the right to use the Pro version for another month (superimposable)')}}</p>
            </div>
            <div class='mt-2' v-if="!loginFlag">
              <span class='contactFree' @click='immediately'>{{$t('Lang.Go immediately')}}</span>
            </div>
        ...
        <div class='w-full flex items-center justify-around text-white py-5 px-6' style='background:#182A77'>
          <div class='flex ml-6'>
            ...
            <div class='flex items-center ml-4'>
              <div>
                <p class='text-lg opacity-75'>{{$t('Lang.Send invitation to friends')}}</p>
                <!-- <p class='opacity-75'>通过分享链接邀请好友注册 {{$t('Lang.')}}</p> -->
              </div>
            </div>
          </div>
          <div>
          ...
          <div class='flex ml-6'>
            ...
            <div class='flex items-center ml-4'>
              <div>
                <p class='text-lg opacity-75'>{{$t('Lang.Friends registration')}}</p>
                <!-- <p class='opacity-75'>好友接受邀请完成注册 {{$t('Lang.')}}</p> -->
              </div>
            </div>
          </div>
          <div>
          ...
          <div class='flex ml-6'>
            ...
            <div class='flex items-center ml-4'>
              <div>
                <p class='text-lg opacity-75'>{{$t('Lang.Get corresponding rewards')}}</p>
                <!-- <p class='opacity-75'>轻松获得对应好礼 {{$t('Lang.')}}</p> -->
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="block lg:hidden border border-solid border-E8E7EB mb-3" v-if="loginFlag">
        ...
        <!-- <div class='mx-4 pb-4'>
          <el-button type="primary"  size="mini" @click="posterBtn" class='w-full h-14 '><span class='text-xl'>{{$t('Lang.Poster invitation')}}</span></el-button>
        </div> -->
    ...
    <!-- 活动规则 -->
    <!-- <div class="border border-solid border-E8E7EB mb-3">
      <div class="flex items-center h-16 lg:h-20 px-3 lg:px-6 border-b border-solid border-E8E7EB" style='background:#F1F2F7'>
        <span class="lg:text-xl font-bold">{{$t('Lang.Activity Rules')}}</span>
      </div>
      <div class='px-6 py-4' style='color:#848599'>
        <p class='my-2 tracking-wider'>1、{{$t('Lang.1Event content During the event all new registered users of the Arockpool browser ')}}</p>
        <p class='my-2 tracking-wider'>2、{{$t('Lang.2User definition New registered users refer to users who have completed registration on ')}}</p>
        <p class='my-2 tracking-wider'>3、{{$t('Lang.3Upper limit of invitation There is no upper limit for the invitation session')}}</p> 
        <p class='my-2 tracking-wider'>4、{{$t('Lang.Activity time from now until')}}</p>
        <p class='my-2 tracking-wider'>5、{{$t('Lang.Event consultation Please scan the QR ')}}</p> 
        <p class='my-2 tracking-wider'>6、{{$t('Lang.Precautions and Disclaimer')}} </p> 
        <div class='lg:flex hidden items-center'>
          <div class='flex-1'>
            <p class='my-2 tracking-wider'>（1）{{$t('Lang.Users are not allowed to participate in this ')}}</p>
            <p class='my-2 tracking-wider'>（2）{{$t('Lang.In the event of force majeure the platform has the right ')}}</p>
            <p class='my-2 tracking-wider'>（3）{{$t('Lang.Within the scope permitted by laws and regulations')}}</p> 
            <p class='my-2 tracking-wider'>（4）{{$t('Lang.The final interpretation right of the event belongs to the Arockpool browser platform')}}</p> 
          </div>
          <div class='flex justify-center mr-6 text-center mb-3'>
            <div>
                <img  src="@/assets/imgs/contact.png" alt="" style='width:120px;height:120px' class="mx-auto">
                <p>{{$t('Lang.Contact Customer Service')}}</p>
      ...
      <div class='flex lg:hidden justify-center mr-6 text-center mb-3'>
        <div>
            <img  src="@/assets/imgs/contact.png" alt="" style='width:120px;height:120px' class="mx-auto">
            <p>{{$t('Lang.Contact Customer Service')}}</p>
            <p>{{$t('Lang.Get professional data guidance')}}</p> 
    ...
    <!-- <div class='poster'>
      <img src="@/assets/imgs/close-btn.png" alt="" width="20" class='absolute cursor-pointer' style='right:-22px;top:-18px'>
      <div id="qrcode" class='absolute' style='right:130px;bottom:45px'></div>
      <img src="@/assets/imgs/poster.png" style='width:320px;height:530px'>
      <el-button>下载海报</el-button>
    </div> -->
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <!-- <el-button @click="posterFlag = false">{{$t('Lang.Cancel')}}</el-button> -->
    ...
    <!-- <el-button @click="toImage">生成截图</el-button> -->
  </div>
</template>
...
import utils from '@/static/utils.js'
  ...
  export default {
    ...
    beforeMount(){
      ...
      // this.http = `${window.location.origin}/${this.lang}/account/transfer?code=${this.get_invite_info.invite_code}`
    ...
    methods:{
      ...
      downLoad(){
        ...
        // console.log(canvas.toBlob("image/png"));
...
.banner-phone{
  height: 300px;
  // background:url('@/assets/imgs/invitation-phone-bg.png') no-repeat;
  background-size: 100% 100%;
  background-color:#09123B;
}
.contact{
