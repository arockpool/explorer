  ...
  <div>
    <div class='lg:flex block justify-between mb-4'>
      ...
      <div class='flex items-center justify-between'>
        ...
        <div class='flex'>
          <div class='lg:w-96 w-50 lg:mr-6 mr-0'>
          ...
          <div class='hidden lg:block'>
            <el-select  v-model="value" size='mini' :placeholder="$t('Lang.Status')" @change='changeStatus' class='w-60'>
              <el-option
    ...
    <div class='mb-3 rounded-md bg-white lg:px-6 px-0 pb-2 mb-4'>
      <!-- <div class='px-2 lg:px-4 pt-8 pb-8 flex items-center justify-between  h-10 border-b border-solid border-E8E7EB title-box'>
        <div>{{$t('Lang.A total of')}} {{total_count}} {{$t('Lang.Deals')}}</div>
        <div class='w-3/5 lg:w-1/4'>
          <el-input clearable v-model="orderSeach" size="mini" :trigger-on-focus="false" @clear='seachBtn' @change="searchChange" :placeholder="$t('Lang.Search Client/Provider/OrderID')">
            <el-button slot="append" size='mini' @click="seachBtn">
              GO
            </el-button>
            ...
            <table class="table-fixed w-full">
              <thead>
                <tr class="">
                  ...
                  <!-- <th class="w-1/12 py-4 font-medium">状态</th> -->
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + '_index'">
                  ...
                  <!-- 客户 -->
                  <td class="w-1/12 py-3 text-center truncate">
                    <!-- <n-link :to="`/${lang}/chain/addr/${item.client}`">{{item.client}}</n-link> -->
                    <el-popover
                      placement="right"
                      trigger="hover"
                      :content="item.client">
...
export default {
  ...
  methods: {
    searchRecordHandler: function(pageIndex) {
      ...
      this.$axios.post('/api/explorer/get_deal_list', {
          ...
          key_words: this.orderSeach,
          is_verified:this.value
        })
        .then(res => {
  ...
  beforeMount(){
    ...
    this.options.unshift({value:2,label:allValue})
  },
