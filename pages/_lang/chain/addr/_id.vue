  ...
  <div class=''>
    <template v-if="type == 'shot'">
          ...
          <ul class="w-full lg:h-64 px-3 lg:px-6 py-0 lg:py-3">
            ...
            <li class="my-3 flex justify-between">
              ...
              <div class="flex items-center justify-between lg:justify-start">
                ...
                <!-- <span class="mr-2 lg:mr-6">{{$t('Lang.All')}} {{minerData.sector_all}}</span> -->
                <!-- 有效 -->
      ...
      <div class="lg:flex mt-3">
        ...
        <div class="mt-3 lg:mt-0 flex-1 lg:border-0 border border-solid border-E8E7EB bg-white rounded-md">
          <div class="flex items-center justify-between h-16 lg:h-20 px-3 lg:px-6 border-b border-solid border-E8E7EB">
            <!-- <div>
              <div class="flex justify-center items-center">
                ...
                <span class="cursor-pointer lg:text-xl  mr-6 text-848599" 
                  @click="blockTitlehandler(item.value)" 
                  :class="{ 'activeTiele' : item.value == blockTitle }" 
                  v-for="(item) in blockTitleList" :key="item.value">
                  {{$t(item.key)}}
                </span>
              </div>
            </div> -->
          ...
          <!-- <div v-show="blockTitle == '奖励'"> -->
            <!-- <div class='h-12 flex items-center justify-end  mr-2 mt-2'>
              
            </div> -->
            <div id="echartsMinerOutputReward" class="h-64 lg:h-96 px-3"></div>
          <!-- </div> -->
          <!-- <div v-show="blockTitle == '出块预测'">
            <div class='h-12 flex items-center justify-end mr-2 mt-2' v-show="blockTitle == '出块预测'">
              <ul class="hidden lg:flex justify-center checkbox-content">
                <li class="checkbox-list cursor-pointer py-1 px-3 lg:px-5 border border-solid border-E6E6EA" @click="blockPredictionhandler(item.value)" :class="{ 'first': index == 0, 'last':  blockPredictionList.length - 1 == index, 'active' : item.value == blockPrediction }" v-for="(item, index) in blockPredictionList" :key="item.value">
                  {{$t(item.key)}}
                </li>
              </ul>
              <div class='w-48  lg:hidden block '>
                <el-select v-model="blockPrediction" placeholder="请选择" >
                  <el-option
                    v-for="item in blockPredictionList"
                    :key="item.value+'___'"
                    :label="$t(item.key)"
                    :value="item.value"
                    >
            ...
            <div id="echartsBlockPrediction" class="h-64 lg:h-84"></div>
          </div> -->
        </div>
      </div>
      ...
      <!-- <div class='mt-3 border border-solid border-E8E7EB'>
        <div class='flex items-center justify-between h-16 lg:h-20 px-3 lg:px-6 border-b border-solid border-E8E7EB'>
          <div class="flex items-center">
              <i class="inline-block bg-3A70FF w-1 h-5 rounded mr-2"></i>
              <span class="lg:text-xl font-bold">{{$t('Lang.Gas Consume')}}</span>
            </div>
            <div class="flex items-center justify-end h-16 lg:h-20 ">
              <ul class="hidden lg:flex justify-center checkbox-content">
                <li class="checkbox-list cursor-pointer py-1 px-3 lg:px-5 border border-solid border-E6E6EA" @click="selectGashandler(item.value)" :class="{ 'first': index == 0, 'last':  gasList.length - 1 == index, 'active' : item.value == gasTime }" v-for="(item, index) in gasList" :key="item.value+'gas'">
                  {{$t(item.key)}}
                </li>
              </ul>
              <div class='w-48 lg:hidden block '>
                <el-select v-model="gasTime" placeholder="请选择" @change='changeGasTime'>
                  <el-option
        ...
        <div class="lg:flex hidden  border-b border-solid border-E8E7EB  items-center justify-between h-28">
          <template v-for="(item, i) in gasListFiveTable">
              ...
              <div class="flex-1 flex items-center justify-center">
                <div>
                  <template>
                    ...
                    <p class="text-848599 text-center h-8">{{ $t(item.title) }}
                      <el-popover v-show="item.popover"  trigger="hover" :content="$t(item.popover)">
                        <i class="el-icon-info cursor-pointer text-gray-500 text-base" slot="reference"></i>
                      </el-popover>
                    </p>
                  </template>
        ...
        <div class="lg:hidden flex border-b border-solid border-E8E7EB  items-center justify-between h-38 pt-4 pb-2 flex-wrap">
          <div v-for="(item, i) in gasListFiveTable" :key="item.title" class='w-1/3 flex'>
            <div v-if="i%3 !== 0" class="bg-E8E7EB h-14 w-px"></div>
             <div class="flex flex-1 justify-center items-center">
              <div class="flex-1 flex items-center justify-center">
                <div>
                  <template>
                    <div class='text-center'>
                      <span class="text-xl">{{ item.val }}</span>
                      <span class="text-sm">{{ item.unit }}</span>
                    </div>
                    <p class="text-848599 text-center px-1" :class="{'h-10' : lang !== 'en','h-12' : lang == 'en'}">{{ $t(item.title) }}
                      <el-popover v-show="item.popover"  trigger="hover" :content="$t(item.popover)">
                        <i class="el-icon-info cursor-pointer text-gray-500 text-base" slot="reference"></i>
                      </el-popover>
                    </p>
        ...
        <div class="">
          <div id="echartsGasConsumption" class="h-64 lg:h-96"></div>
        </div>
      </div> -->
      ...
      <el-dialog :title="$t(dialogTitle)" :visible.sync="tagFlag" width="648px" :before-close="handleClose">
        ...
        <el-form label-position="top" label-width="140px" ref="tagForm" size="mini" :model="tagForm" :rules="rules">
          ...
          <el-form-item :label="$t('Lang.Message')">
            <!-- <el-input type="textarea" disabled v-model="tagForm.msgarea"></el-input> -->
            <div class='px-6 py-2 text-xs' style='background:rgba(247, 248, 252, 1);color:rgba(119, 140, 162, 1)'>
              <p class='truncate'>Signature for arockpool {{miner_no}} {{nowTime}}</p>
            </div>
          </el-form-item>
      ...
      <div class="mt-3 lg:border-0 border border-solid border-E8E7EB bg-white rounded-md">
        ...
        <ul class="py-3 px-3 lg:px-6">
          <li class="flex items-center justify-between lg:justify-start leading-8" v-for="(item, index) in walletOverview" :key="index + '_index'">
            <!-- <template v-if="!item.type || address_type.indexOf(item.type) > -1">
              <div class="w-48 text-848599 flex-shrink-0">{{$t(item.title)}}</div>
              <div class="truncate lg:break-all " v-if="item.title == 'Lang.Types'">{{$t(item.val)}}</div>
              <div class="truncate lg:break-all " v-else>{{item.val}}</div>
            </template> -->

            <!-- <div> -->
              <!-- <div class="w-48 text-848599 flex-shrink-0" v-if="item.val!=='--'">{{$t(item.title)}}</div>
              <div class="truncate lg:break-all" v-if="!item.node && item.val!=='--'">{{item.val}}</div>
              <div  v-if="item.node">
                <div v-if='item.val.length!==0' class="flex items-end flex-wrap">
                  <n-link v-for="(i,j) in item.val" :key="j+'node'" class="text-main break-all ml-6 lg:ml-0 lg:mr-6 mr-0" :to="`/${lang}/chain/addr/${i}`">{{i}}</n-link>
                </div>
                <p v-else>--</p>
              </div> -->
    ...
    <div id='shareBox' ref="imageWrapper" class='bg-main px-4 py-6 text-white' >
      ...
      <div class='flex justify-between'>
        <!-- <img class="h-8 " v-show="lang == 'zh'"  src="~/assets/img/icon-logo.png" />
        <img class="h-8 " v-show="lang !== 'zh'"  src="~/assets/img/icon-en-logo.png" /> -->
        ...
        <div class="text-main">
          <ul class="flex flex-col justify-between w-full  px-3  py-3">
            ...
            <li class="flex-grow flex justify-between mb-2">
              ...
              <div class="">
                <!-- <p class="mr-2">{{$t('Lang.All')}} {{minerData.sector_all}}</p> -->
      ...
      <div class='flex items-start justify-between mt-8'>
        <div>
          ...
          <p calss='text-xs'>{{$t('Lang.In the Arockpool browser')}}(arockpool.com)</p>
        </div>
        <div class='text-center'>
      ...
      <!-- <img class="h-8 -mt-6" v-show="lang == 'zh'"  src="~/assets/img/icon-logo.png" />
      <img class="h-8 -mt-6" v-show="lang !== 'zh'"  src="~/assets/img/icon-en-logo.png" /> -->
  ...
  computed: {
    ...
    minerCost : function() {
      let {
        total_pledge,// 扇区质押
        create_gas,// 单t生产gas
        create_gas_overview, // 生产gas参考
        total_gas, // gas消耗

        // 总成本 = total_gas+total_pledge
        // 差值 = create_gas-create_gas_overview 
      ...
      return [{
        ...
        // val: Number(total_gas)+Number(total_pledge),
        unit: 'F',
      },{
        title: 'Lang.Sector pledge',
        // popover: 'Lang.扇区质押',
        val: utils.formatTrueFil(total_pledge, true),
        // unit: utils.formatTrueFil(Number(total_pledge), true)
      },{
        title: 'Lang.Single T production gas',
        // popover: 'Lang.单t生产gas',
        ...
        // unit: utils.formatEasyFil(total_gas, false).split(' ')[1]
      },{
    ...
    minerOverview: function() {
      ...
      return [
        ...
        // { title: 'Lang.Location_2', val: ip ? ip : 'N/A' },
      ]
    },
    walletOverview: function() {
      ...
      //   worker_id,
      //   worker_address,
      //   worker_balance_value,

      //   synced_at_str
      // } = this.walletData
      // return [
      //   { title: 'Lang.Address', val: owner_address, type: 'owner' },
      //   { title: 'Lang.Address', val: post_address, type: 'post' },
      //   { title: 'Lang.Address', val: worker_address, type: 'worker' },

      //   { title: 'Lang.ID', val: owner_id, type: 'owner' },
      //   { title: 'Lang.ID', val: poster_id, type: 'post' },
      //   { title: 'Lang.ID', val: worker_id, type: 'worker' },

      //   { title: 'Lang.Types', val: 'Lang.Ordinary account' },

      //   { title: 'Lang.Balance', val: utils.formatTrueFil(owner_balance_value, true,8), type: 'owner' },
      //   { title: 'Lang.Balance', val: utils.formatTrueFil(post_balance_value, true,8), type: 'post' },
      //   { title: 'Lang.Balance', val: utils.formatTrueFil(worker_balance_value, true,8), type: 'worker' },

      //   { title: 'Lang.synchronised time', val: synced_at_str }
      // ]
      let {
        // 地址
        address,
    ...
    gasListFiveTable:function(){
      ...
      return [
        ...
        { title: 'Lang.Sector pledge',
          // val: day_pledge_gas,
          // unit:'FIL',
          // val: utils.formatEasyFil('1056.763364394441135126'),
          val: utils.formatEasyFil(day_pledge_gas).split(' ')[0],
          unit: utils.formatEasyFil(day_pledge_gas).split(' ')[1],
        },
        { title: 'Lang.Gas Consume',
          val: day_total_gas,
          // val: utils.formatNewEasyFil(day_total_gas)[0],
          // unit:utils.formatNewEasyFil(day_total_gas)[1]
          unit:'F',
        },
        { title: 'Lang.Gas Used',
          val: day_create_gas,
          // val: utils.formatTrueFil(day_create_gas),
          unit:'F/TiB',
        },
        { 
          title: 'Lang.Gas Used Reference',
          val:  day_overview_create_gas,
          // val:  utils.formatTrueFil(day_overview_create_gas),
          unit:'F/TiB',
          popover:'Lang.Average Gas Production in the Whole Network'
        },
        { 
          title: 'Lang.Gas Difference',
          val: day_gas_offset,
          // val: utils.formatTrueFil(day_gas_offset),
          unit:'F/TiB',
          popover:'Lang.Production Gas minus production Gas reference value'
        },
    ...
    return app.$axios.post('/api/explorer/search_miner_or_wallet', {
      ...
      .then(res => {
        ...
        // 如果是地址
          ...
          // return app.$axios.post('/api/explorer/search_miner_type', {
          //   value: data.miner_no
          // })
      ...
      .then(res => {
        ...
        if (data.type == 'shot') {
          ...
          // return app.$axios.post("/api/explorer/get_miner_gas_by_no",{
          //   miner_no:params.id,
          //   stats_type:'7d'
          // });
          return data
  ...
  methods: {
    ...
    initEchartsMinerBalance: function() {
      ...
      echarts.setOption(echartsUtils.optionsForMinerBalance({
        balance: this.minerData.available_balance, // 可用
        init_pledge: this.minerData.initial_pledge_balance, // 扇区质押
        locked_pledge_balance: this.minerData.locked_balance, // 挖矿质押
    ...
    selectGashandler(item){
      ...
      // this.$axios.post("/api/explorer/get_miner_gas_chart_by_no",{
      //   miner_no:this.miner_no,
      //   stats_type:item
      // }).then(res=>{
      //   this.gasConsumptionList = res.data
      //   this.initEchartsGasConsumption()
      // })
    },
    changeGasTime(){
        ...
        html2canvas(myBox, {
          ...
          if(data.length < 10){
            ...
            image.onload = function () {
              ...
              a.download = that.miner_no+'存储供应商详情'; // 设置图片名称
    ...
    toImage() {
      ...
      // a.href = this.imgUrl;
      // a.setAttribute('download', '算力封装产出');
      // a.click();
      let that = this
      var image = new Image();
      image.setAttribute("crossOrigin", "anonymous");
