      ...
      <div v-show="objs.length !== 0">
          ...
          <table class="table-fixed w-full">
            <thead>
              <tr>
                ...
                <!-- <th class="w-1/12 py-4 font-medium text-6C757E">{{$t('Lang.Loss of computing power')}}</th> -->
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + 'data_index'">
                ...
                <!-- <td class="w-1/12 py-3 text-center text-dark">{{item.increase_power_loss | formatPower(2)[0] }}{{item.increase_power_loss | formatPower(2)[1] }}</td> -->
              </tr>
            </tbody>
          </table>
        </div>
        <div class="lg:hidden block bg-white mt-3">
          <div v-for="(item, index) in objs" class="p-3 mb-3  rounded-md bg-white border border-solid border-E8E7EB" :key="index + 'data_index'">
            ...
            <!-- <div class="flex items-center justify-between pb-1">
              <span>{{$t('Lang.Loss of computing power')}}</span>
              <span>{{item.increase_power_loss | formatPower(2)[0] }}{{item.increase_power_loss | formatPower(2)[1] }}</span>
            </div> -->
          </div>
        </div>
