<template>
   <div class="pb-3">
    <!-- 扇区质押 -->
    <p class='lg:block hidden text-2xl mb-3'>{{$t('Lang.Sector pledge')}}</p>
    <!-- pc扇区质押趋势图 -->
    <div class="lg:block hidden rounded-md bg-white">
      <div class="flex items-center justify-between h-20 px-3 lg:px-6 border-b border-solid border-E8E7EB">
        <div class="flex items-center">
          <span class="lg:text-lg font-medium text-dark">{{$t('Lang.Sector pledge trend chart')}}</span>
        </div>
      ...
      <div id="echartsSector" class="h-64 lg:h-96 px-6"></div>
    </div>
  
    <!-- phon实时扇区质押 -->
    <div class="block lg:hidden  rounded-md lg:bg-white bg-background mb-6">
      <div class="lg:flex block items-center justify-between lg:h-20 lg:px-6 lg:border-b border-solid border-E8E7EB">
        <div class="flex items-center">
          <span class="lg:text-lg text-xl font-medium text-dark mb-2">{{$t('Lang.Real time sector pledge')}}</span>
        </div>
      </div>
    ...
    <div class="lg:mt-3 mt-14 rounded-md bg-white">
      <div class="lg:flex block items-center justify-between lg:h-20 lg:px-6 lg:border-b border-solid border-E8E7EB">
        <div class="flex items-center">
          <span class="lg:text-lg text-xl font-medium text-dark mb-2">{{$t('Lang.Historical sector pledge')}}</span>
        </div>
        <div class='flex'>
           <el-date-picker
            v-model="date"
          ...
          <table class="table-fixed w-full">
            ...
            <tbody>
              <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + 'data_index'">
                ...
                <td class="w-1/12 py-3 text-center text-dark">{{item.avg_pledge | formatTrueFil}} F/TiB</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="lg:hidden block bg-white mt-3">
          <div v-for="(item, index) in objs" class="p-3 mb-3  rounded-md bg-white border border-solid border-E8E7EB" :key="index + 'data_index'">
            ...
            <div class="flex items-center justify-between pb-1">
              ...
              <span>{{item.avg_pledge | formatTrueFil}} F/TiB</span>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    dataOverviewStat(){
      ...
      return [
        {
          title: 'Lang.Current Sector Pledge',
  ...
  data: function () {
    return {
      time: "day",
      // checkboxList: ['Lang.24 hours', 'Lang.Latest seven days', 'Lang.Latest one month', 'Lang.Latest three months'],
      checkboxList: [
  ...
  methods: {
    ...
    initEchartsEfficiencyHandler: function() {
      ...
      if (this.lang == 'zh') {
        ...
        echartTitle = ['セクターのPledge']
      } else {
        echartTitle = ['섹터 서약']
      }

      this.line.map(item => {
        y1.push(utils.formatTrueFil(item.avg_pledge, 4).split(' ')[0])
        date.push(item.height_time)
      })
      let echarts = this.$echarts.init(document.getElementById('echartsSector'))
