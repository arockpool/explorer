          ...
          <table class="table-fixed w-full">
            <thead>
              <tr>
                ...
                <!-- <th class="w-1/12 py-4 font-medium text-6C757E">{{$t('Lang.FiL price')}}</th> -->
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + 'data_index'">
                ...
                <td class="w-1/12 py-3 text-center text-dark">
                  <p v-if="item.rmd_avg_reward == 0">--</p>
                  <p v-else>{{item.rmd_avg_reward | formatTrueFil(true)}}/TiB</p>
                </td>
                <td class="w-1/12 py-3 text-center text-dark">
                  <p v-if="item.rmd_avg_pledge == 0">--</p>
                  <p v-else>{{item.rmd_avg_pledge | formatTrueFil(true)}}/TiB</p>
                </td>
                <td class="w-1/12 py-3 text-center text-dark">
                  <p v-if="item.rmd_create_gas_32 == 0">--</p>
                  <p v-else>{{item.rmd_create_gas_32 | formatTrueFil(true)}}/TiB</p>
                </td>
                <td class="w-1/12 py-3 text-center text-dark">
                  <p v-if="item.rmd_create_gas_64 == 0">--</p>
                  <p v-else>{{item.rmd_create_gas_64 | formatTrueFil(true)}}/TiB</p>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div class="lg:hidden block bg-white">
          <div v-for="(item, index) in objs" class="p-3 mb-3  rounded-md bg-white border border-solid border-E8E7EB" :key="index + 'data_index'">
            ...
            <div class="flex items-center justify-between pb-1">
              ...
              <!-- <span>{{item.rmd_avg_reward | formatTrueFil(true)}}/TiB</span> -->
              <span v-if="item.rmd_avg_reward == 0">--</span>
              <span v-else>{{item.rmd_avg_reward | formatTrueFil(true)}}/TiB</span>
            </div>
            <div class="flex items-center justify-between pb-1">
              <span>{{$t('Lang.Sector pledge')}}</span>
              <!-- <span>{{item.rmd_avg_pledge | formatTrueFil(true)}}/TiB</span> -->
              <p v-if="item.rmd_avg_pledge == 0">--</p>
              <p v-else>{{item.rmd_avg_pledge | formatTrueFil(true)}}/TiB</p>
            </div>
            <div class="flex items-center justify-between pb-1">
              <span>{{$t('Lang.32G production Gas')}}</span>
              <!-- <span>{{item.rmd_create_gas_32 | formatTrueFil(true)}}/TiB</span> -->
              <p v-if="item.rmd_create_gas_32 == 0">--</p>
              <p v-else>{{item.rmd_create_gas_32 | formatTrueFil(true)}}/TiB</p>
            </div>
            <div class="flex items-center justify-between pb-1">
              <span>{{$t('Lang.64G production Gas')}}</span>
              <!-- <span>{{item.rmd_create_gas_64 | formatTrueFil(true)}}/TiB</span> -->
              <p v-if="item.rmd_create_gas_64 == 0">--</p>
              <p v-else>{{item.rmd_create_gas_64 | formatTrueFil(true)}}/TiB</p>
            </div>
            <!-- <div class="flex items-center justify-between pb-1">
              <span>{{$t('Lang.FiL price')}}</span>
  ...
  computed: {
    ...
    overview_stat(){
      ...
      if(this.objs.length == 0){
        ...
        rmd_create_gas_32 = this.objs[0].rmd_create_gas_32;
        rmd_create_gas_64 = this.objs[0].rmd_create_gas_64;
      }
      ...
      return [
        ...
        // 32GiB生产Gas(昨日)
        {
          ...
          val: utils.formatTrueFil(rmd_create_gas_32) || '--',
          unit: 'F/TiB'
        },
        // 64GiB生产Gas(昨日)
        {
          title: 'Lang.64GiB production gas(Yesterday)',
          val: utils.formatTrueFil(rmd_create_gas_64) || '--',
          unit: 'F/TiB'
        },
        // {
