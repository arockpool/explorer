  ...
  <div class="pt-3">
    ...
    <div class="border border-solid border-E8E7EB">
        ...
        <div class="flex-1 flex items-center lg:my-0 my-2">
          ...
          <template v-if="globalMinerTag[miner_exponent.miner_no]">
            <div class="border border-solid border-E8E7EB rounded mr-3 flex items-center">
              ...
              <!-- 显示认证标记 -->
              <!-- <img v-show="globalMinerTag[miner_exponent.miner_no].signed"  src="@/assets/imgs/isSigned.png"  alt=""  class='mr-2'  style='width:12px;height:12px'> -->
      ...
      <ul class="flex flex-wrap">
        ...
        <!--         <li class="w-1/2 lg:flex-1 flex items-center h-20 lg:h-24">
          <div class="flex-1 flex flex-col items-center">
            <div>{{miner_exponent.miner_type == 1 ? '大于10PiB' : '小于10PiB' }}</div>
            <div class="text-75737F mb-1">算力规模</div>
          </div>
        </li> -->
      </ul>
  ...
  methods: {
    ...
    minerIndexTrendHandler: function() {
      ...
      this.dates.map(item => {
        ...
        if (Object.keys(data).length) {
          date.push(item) // x轴日期
          y1.push(data.all) // all 全网数据
          y2.push(data.top10) // top10
          y3.push(data.top30) // top30
          y4.push(data.miner) // miner
        }
      })
    ...
    minerDimensionalityHandler: function() {
      ...
      */
      ...
      let keyOrder = [
        'total_power_i', // 有效算力
        'avg_reward_i', // 24h挖矿效率
        'power_increment_7day_i', // 7日算力增长量
        'keep_gas_week_i', // 7日算力维护Gas
        'section_fault_rate_i' // 7日扇区错误率
      ]
