<template>
  <div class="mb-16">
    ...
    <el-dialog
        ...
        <el-form label-position="top" label-width="80px"  size="mini">
          <el-form-item :label="$t('Lang.Miner')" >
            <!-- <el-input  v-model="nodeTwo" :placeholder="$t('Lang.Please enter')" clearable @keyup.enter.native="enterKey"></el-input> -->
          </el-form-item>
        </el-form>
        <div class='-mt-6'>
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <!-- <el-button type="primary" @click="okTwo">确 定</el-button> -->
    ...
    <el-dialog
      ...
      <el-form label-position="top" label-width="80px"  size="mini">
        <el-form-item :label="$t('Lang.Miner')" >
          <!-- <el-input  v-model="nodeThree" :placeholder="$t('Lang.Please enter')" clearable  @keyup.enter.native="enterKey"></el-input> -->
        </el-form-item>
      </el-form>
      <div class='-mt-6'>
        ...
        <div class="container text-white mx-auto flex flex-col justify-center h-full text-opacity-75 lg:px-10 px-3">
          <div class='flex lg:py-10 pt-4 pb-3 justify-between items-center'>
            <div class="lg:w-1/2">
              <div class="flex">
                <n-link to="">
                  <!-- <img class="h-8 lg:h-10" src="~/assets/img/icon-logo.png" /> -->
...
export default {
    ...
    asyncData({params,app,redirect}) {
      let data = {
        ...
        miner_no:'',
        tableBox:[
          {subTitle:'Lang.Overall score​',dataList:['','','']},//0综合评分
          {subTitle:'Lang.Operating status',dataList:['','',''],lang:true},//1运行状态
          {subTitle:'Lang.Production Efficiency',dataList:['','',''],lang:true},//2产出效率
          {subTitle:'Lang.New computing power cost',dataList:['','',''],lang:true},//3新增算力成本
          {subTitle:'Lang.Effective computing power',dataList:['','','']},//4有效算力
          {subTitle:'Lang.Proportion of effective computing power',dataList:['','','']},//5有效算力占比
          {subTitle:'Lang.Sector size',dataList:['','','']},//6扇区大小
          {title:'Lang.Profit analysis'},//7产出分析
          {subTitle:'Lang.Production Efficiency in 24 hours',dataList:['','','']},//8 24h产出效率
          {subTitle:'Lang.24h bursts',dataList:['','','']},//9 24h爆块数
          {subTitle:'Lang.Rewards in 24 hours',dataList:['','','']},//10 24h验证成功奖励
          {echartsTitle:'Lang.Output efficiency trend',echarts:true,id:'outputEfficiency'},//11 产出效率趋势
          {title:'Lang.Cost analysis'},//12 成本分析
          {subTitle:'Lang.24h Gas used',dataList:['','','']},//13 24h生产Gas
          {subTitle:'Lang.24h maintenance Gas',dataList:['','','']},//14 24h维护Gas
          {subTitle:'Lang.24h sector pledge',dataList:['','','']},//15 24h维护Gas
          {echartsTitle:'Lang.Production Gas Trend',echarts:true,id:'productionGasTrend'},//16 生产Gas趋势
          {title:'Lang.Burst analysis'},//17 爆块分析
          {subTitle:'Lang.24h Lucky value',dataList:['','','']},//18 24h幸运值
          {subTitle:'Lang.24h burst rate',dataList:['','','']},//19 24h爆块率
          {echartsTitle:'Lang.Lucky value trend',echarts:true,id:'luckyTrend'},//20 幸运值趋势
          {title:'Lang.Wallet'},//21 钱包
          {subTitle:'Lang.workers wallet balance',dataList:['','','']},//22 worker账户钱包余额
          {subTitle:'Lang.Expected to use',dataList:['','','']},//23 预计使用
          {subTitle:'Lang.posts wallet balance',dataList:['','','']},//24 post账户钱包余额
          {subTitle:'Lang.Expected to use',dataList:['','','']},//25 预计使用
          {title:'Lang.Gas analysis'},//26 Gas分析
          {subTitle:'Lang.Gas total cost',dataList:['','','']},//27 Gas总费用
          {subTitle:'Lang.PreCommitSector',dataList:['','','']},//28 PreCommitSector
          {subTitle:'Lang.ProveCommitSector',dataList:['','','']},//29 ProveCommitSector
          {subTitle:'Lang.SubmitWindowedPoSt',dataList:['','','']},//30 SubmitWindowedPoSt
          {subTitle:'Lang.Other',dataList:['','','']},//31 其他
          {title:'Lang.Message quality analysis'},//32 消息质量分析
          {subTitle:'Lang.PreCommitSector',dataList:['--','','']},//33 PreCommitSector
          {subTitle:'Lang.ProveCommitSector',dataList:['--','','']},//34 ProveCommitSector
          {subTitle:'Lang.SubmitWindowedPoSt',dataList:['--','','']},//35 SubmitWindowedPoSt
          // {title:'FIL罚金'},//36
          // {subTitle:'全部罚金',dataList:[]},//36
        ],
        objs:[],
        user_info:{},
        get_overview:{},
      ...
      return app.$axios.post("/api/explorer/get_user_info").then(res=>{
        ...
        // data.tableBox[3].dataList[0] = utils.formatEasyFil(res.data.cost_dict.cost) 
        // data.tableBox[2].dataList[0] = res.data.avg_reward
        ...
        // data.tableBox[15].dataList[0] = res.data.pledge_gas ? utils.formatterAmount(res.data.avg_pledge * 32)+' F/TiB' : '--'
    ...
    methods: {
      ...
      addNodeOk(miner_no,index){
        ...
        Promise.all([
          ...
          // console.log(res);
          // ((data.miner_health_report_24h.power / res.data.power)*100).toFixed(2)+' %'
          // console.log(this.get_overview.power,res[0].data.power);
          // console.log(res[0]);
        ...
        // this.tableBox[15].dataList.splice(index,1,this.get_overview.avg_pledge ? utils.formatterAmount(this.get_overview.avg_pledge * 32)+' F/TiB' : '--')
        // this.tableBox[15].dataList.splice(index,1,res[0].data.pledge_gas)
        // console.log(res);
      ...
      outputEfficiencyEchart(){
        ...
        window.addEventListener("resize", () => {
          echarts.resize();//resize 页面大小改变，调整大小
        });
        // console.log(y1)
      },
      // 产出效率趋势导出
      ...
      productionGasTrendEchart(){
        ...
        let y1 = [],
          ...
          if(this.objsTwoNode.length !== 0){
            this.objsTwoNode.map(i=>{
              // y2.unshift(i.create_gas)
              y2.unshift(utils.formatTrueFil(i.create_gas))
            })
          }
          if(this.objsThreeNode.length !== 0){
            this.objsThreeNode.map(i=>{
              // y3.unshift(i.create_gas)
