<template>
  <div class="flex flex-col items-center border-t border-solid border-E6E6EA py-10 lg:py-20 rounded-b-lg mb-6">
    <div class="px-5 w-full lg:w-148">
      ...
      <!-- 邀请码 -->
      <!-- <div class="mt-5 mb-4 text-gray-800 text-lg font-medium">{{$t('Lang.Invitation code')}} ({{$t('Lang.Optional')}})</div>
      <el-input @focus='inputFocus' clearable v-model="registerForm.invitation" class="mb-10 login-el-input" :placeholder="$t('Lang.Please enter')" ></el-input> -->
  ...
  methods: {
    ...
    getWebVerification() {
      this.$axios.post("/api/system/get_str").then(res => {
        ...
        // console.log('解密',utils.getDAes(this.verification));
      })
    },
    // 获取手机验证码
    getSmsLoginvercode() {
    ...
    ok() {
      ...
      // if( this.verification !== this.webVerification){
      //   this.errorMsg = 'Lang.Graphic verification code error'
      //   this.refreshImgHandler()
      //   return
      // }
    ...
    getUser() {
      ...
      // this.$axios.post('/api/explorer/update_pro_tips_flag').then(res=>{
      //   console.log('update_pro_tips_flag',res);
      // })
    },
    inputFocus() {
      this.errorMsg = "";
