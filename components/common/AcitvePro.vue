  ...
  <div class='activePro mb-3'>
    ...
    <div>
            ...
            <template v-if="collectionNodeList && collectionNodeList.length !== 0">
              <div class='mt-1' v-show="!lookOtherFlag">
                <div class='lg:mx-6 mx-0 lg:my-0 my-3 flex items-center w-48'>
                  <!-- <p class='mr-2'>{{$t('Lang.My Collection')}}:</p> -->
        ...
        <div class="lg:flex">
          ...
          <div class="flex-1 rounded-md bg-white lg:border-0 border border-solid border-E8E7EB">
            <div class="flex items-center h-16 lg:h-20 px-3 lg:px-6 border-b border-solid border-E8E7EB">
              <!-- <i class="inline-block bg-F4BF75 w-1 h-5 rounded mr-2"></i> -->
            ...
            <div class='px-6 my-4'>
              <div class='' style='background:#F9F9FD'>
                <p class='pl-10 py-4' >
                  <!-- 当前产出效率正常{{objsOne.avg_reward_warn}} -->
              ...
              <div class='' style='background:#F9F9FD'>
                <p class='pl-10 py-4'>
                  <span v-show="objsOne.gas_warn" style='color:#FA9300'>
                    <!-- {{$t("Lang.The production gas and maintenance Gsa of the node for 7 consecutive days are higher than 30% of the average of the whole network")}} -->
                ...
                <div class='w-full text-center px-6'>
                  ...
                  <div class='flex items-center justify-around'>
                    <div v-for="(k,m) in item.sublist" :key="m+'msg'" style='width:23%'>
                      <!-- <div v-if="m > 0" class="bg-E8E7EB h-10 w-px" :key="item.title + 'line'"></div> -->
      ...
      <!-- <ul class="hidden lg:flex justify-center py-5">
        <li >
          <p class="proChild-link-active block px-6 py-2 mx-3 text-lg border border-solid border-E6E6EA rounded font-medium hover:text-6F4715 hover:border-6F4715"
          ></p>
        </li>
      </ul> -->
                ...
                <tbody>
                  <tr v-for="(item, index) in objs" class="border-b border-solid border-E8E7EB" :key="index + '_index'">
                    <td class="w-1/12 py-3 text-center pl-5">
                      <!-- <n-link :to="`/${lang}/chain/msg/${item.msg_cid}`">{{item.msg_cid | getProSubStr}}</n-link> -->
                      <el-popover
                        placement="right"
                        trigger="hover"
                    ...
                    <td class="w-2/12 py-3 text-center truncate">
                      <div class='flex items-center justify-center'>
                        ...
                        <!-- <div class="border border-solid border-E8E7EB rounded ml-3 flex items-center" v-if="item.msg_to_tag[0]">
                            <span class="mx-3 text-75737F" >{{item.msg_to_tag[0]}}</span>
                            <img v-show="item.msg_to_tag[2]" src="../../../../../../assets/imgs/isSigned.png" alt="" class='mr-2' style='width:12px;height:14px'>
                        </div> -->
                      </div>
                    </td>
    ...
    <!-- <img :src="imgUrl" alt="" style='width:1260px'> -->
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    totalScore_term(){
      ...
      return {
        worker_estimated_day,//worker使用天数
        poster_estimated_day,//poster使用天数
        wallet_score,//钱包分析
        gas_score,//成本分析
        lucky_score,//爆块分析
        status_dict,//运行状态
        avg_reward_dict,//产出效率
        cost_dict //新增算力成本
      }
    },
    // 收益分析
    ...
    costHealthList(){
      ...
      // console.log(PreCommitSector);


      // console.log(PreCommitSectorBatch);
      // console.log(PreCommitSectorBatch.count);
      // console.log(PreCommitSectorBatch.error_count);
      // console.log(PreCommitSectorBatch.ok_count);
  ...
  methods: {
    ...
    revenueAnalysisEchart(){
      ...
      // console.log('objsThree',this.objsThree)
    ...
    costechart_keep_gas(){
      ...
      this.objsThree.map(item => {
        // y1.unshift(utils.formatTrueFil(item.keep_gas))
        // y2.unshift(utils.formatTrueFil(item.keep_gas_overview))
    ...
    costechart_create_gas(){
      ...
      this.objsThree.map(item => {
        ...
        // y2.unshift(utils.formatTrueFil(item.create_gas_overview))
    ...
    searchRecordHandler: function(pageIndex) {
      ...
      this.$axios.post('/api/explorer/get_transfer_list_by_no', {
        ...
        .then(res => {
          ...
          this.objs = res.data.objects
          this.loadingFlag = false
          // this.total_count = res.data.total_count > 9800 ? 9800 : res.data.total_count
    ...
    toImage() {
      ...
      image.onload = function () {
        ...
        a.download = that.collectionNode+'存储供应商健康报告'; // 设置图片名称
